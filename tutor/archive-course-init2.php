...
// echo "<pre>";
// print_r($GLOBALS['tutor_course_archive_arg']);
// echo "</pre>";
		...
		<ul class="tutor-pagination-numbers">
			...
			for($i = 1; $i <= $pages_count; $i++) {
				if($i == $current_page) {
					...
					echo '<a aria-label="Page ' . esc_attr($i) . '" class="page-numbers" href="' . '?current_page='.$i.'">' . esc_html($i) . '</a>';
				}
			}

			if($current_page < $pages_count) {
				echo '<a class="next page-numbers" href="' . esc_url(add_query_arg('current_page', ($current_page + 1), $current_url)) . '"><span class="tutor-icon-angle-right"></span></a>';
			}
			?>		
		</ul>
	</nav>	
